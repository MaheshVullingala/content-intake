...
Not yet built: the actual Okta↔GoTrue SAML configuration (blocked on the VM/DNS name).

`sql/18` and `sql/19` have been applied live and verified (column, RPC, and trigger fix all confirmed present in the live DB).

## Invite User admin flow (2026-08-08)

Closes the loop on the original ask: an admin who knows someone's email can add them and assign a role ahead of time, they get notified, and the role is already in place the moment they first log in — whether that's via self-registration (today) or Okta (once it's live). Deliberately does *not* use Supabase's admin-invite-by-email API (which would create the `auth.users` row immediately and require a separate password-set flow) — simpler to lean on the registration path that already exists: the invite email tells the person to register (or sign in with Okta, if enabled) using this same email address, and `handle_new_user()`'s email-linking fix (sql/18) takes it from there automatically.

- `sql/20-invite-user.sql` (+ `sql/00-consolidated-bootstrap.sql`) — `public.invite_user(p_email, p_name, p_department, p_role, p_can_assign)`, a `SECURITY DEFINER` RPC (needed because `public.users` has no INSERT policy at all — the only other writer is the trigger). Self-checks the caller is `role = 'admin'`, validates the email ends in `@cadence.com`, validates the role against the same assignable set AdminPanel's role dropdown offers, and refuses to create a duplicate if a row for that email already exists. Inserts with `auth_id` left `NULL`.
- `src/lib/email.js` — new `inviteEmailHtml({ name, roleLabel, appUrl, oktaEnabled })` template. Wording branches on whether Okta is enabled: "sign in with Okta" vs. "create an account with this same email."
- `src/app/api/admin/invite-email/route.js` (new) — server-only, sends the notification via the existing SMTP infra. Independently re-verifies the caller is an admin from their access token (same pattern as `/api/audit`) rather than trusting the client, since this is a separate network call from the RPC that actually created the row. Gated by `settings.email_notifications_enabled` same as every other notification email — if it's off, the row is still created (that's the source of truth), the response just reports `skipped` with a reason so the admin knows to share access manually.
- `src/components/AdminPanel.js` — "+ Invite User" button in the Users tab header opens a modal (email, name, department, role, can-assign). On submit: calls the RPC, prepends the new row to the users list, then best-effort POSTs to the email route and reports whether it actually sent. Users tab table now shows a small "Invited" badge next to the email for any row with `auth_id IS NULL`, so it's visible at a glance who hasn't logged in yet.

## Fill Test Data: temporary production override (2026-08-08)

Asked to keep the button working in production for a while (QA on a live-configured deployment before real launch), rather than only ever available in dev. Added a second, admin-controlled gate on top of the existing `NODE_ENV` one instead of just removing the `NODE_ENV` check — the goal is "on for now, easy to turn back off," not "on forever again."

- `settings.test_data_enabled BOOLEAN DEFAULT true` (`sql/21-test-data-toggle.sql` + `sql/00-consolidated-bootstrap.sql`). No new RPC needed — unlike the password-login flag, `NewRequest.js` only reads this after the user is already logged in, so the existing `settings_select` RLS (`auth.uid() IS NOT NULL`) already covers it.
- `NewRequest.js` — button condition is now `NODE_ENV !== "production" || testDataEnabled`. In dev it always shows regardless of the flag (unchanged behavior). In production, it only shows if the flag is on — fetched once on mount, defaults to `true` locally so it doesn't flicker away while loading.
- `AdminPanel.js` — new "Fill Test Data Button" toggle in Settings, same pattern as the other two. Copy on the card is explicit that this is a temporary QA convenience, not a safety-net weakening: the pre-flight check's placeholder-text detection still hard-blocks Submit either way, so turning this on doesn't let anyone actually publish Lorem Ipsum content — it only affects whether the fill-shortcut button itself is visible.
- One tradeoff worth knowing: since the gate is now partly runtime (DB-backed) rather than purely build-time, `fillTestData()` and the button JSX can no longer be fully dead-code-eliminated from the production bundle the way they were before — the code ships either way now, just conditionally rendered. Confirmed via build output (bundle grew from 259kB → 263kB First Load JS on `/`).

## Production deploys were silently blocked for 3 weeks — root cause found (2026-08-11)

User reported the Vercel-hosted app wasn't showing recent changes. Initial investigation (via the Vercel MCP once reconnected) found something worse than a stale branch: **no deployment had succeeded since v159 (`deafaab`, 2026-07-19)** — every one of the 8 commits since (v160 through v167) had been pushed to `main` correctly but never built. First hypothesis was a broken GitHub↔Vercel webhook; reconnecting the Git integration in the Vercel dashboard didn't fix it, which ruled that out.

Actual cause, found via a manual "Create Deployment" attempt in the Vercel dashboard: `vercel.json`, introduced in **v160** (the very first commit after the last successful deploy), scheduled the notification-sweep cron every 5 minutes (`*/5 * * * *`). Vercel's Hobby plan only allows daily cron jobs — every deployment attempt since v160, automatic (webhook) or manual, was being rejected at the cron-validation step before it ever reached the build step. This is why reconnecting the webhook changed nothing: the webhook was never broken, every triggered deployment was failing the same validation check.
...
## Slow/timing-out stakeholder brand-image approval (2026-08-11)

Reported as: approving Brand's uploaded images from the stakeholder side sometimes takes a long time or times out. Traced to `TaskBoardOverview.js`'s `handleApprove(task, "stakeholder_approved_brand")` — the click handler had **3 sequential, awaited network round-trips** before the "Saving…" state cleared: the task status update, the `requests` update (setting `stakeholder_approved_brand = true`), and — the one that shouldn't have been there — a `SELECT id FROM users WHERE role = 'design_team'` to build the notify-design-team list.

That third call had a comment directly above it claiming "Fire and forget — never let a notification failure ... block or delay the approval flow" — but the code didn't match the comment: only the `notifications` INSERT after it was fire-and-forget; the `SELECT` feeding it was still `await`ed, sitting squarely in the critical path. On a Supabase Free-tier project (cold-start / connection-pool latency after idle periods is a known Free-plan characteristic), that extra synchronous round-trip was the most likely thing pushing occasional requests past `src/lib/supabase.js`'s 10-second client-side abort timeout.

Fix: wrapped the whole design_team-notify block (`SELECT` + `INSERT`) in a self-invoking async function that's never awaited by `handleApprove` — genuinely fire-and-forget now, matching what the comment always said it was. Reduces the critical path from 3 sequential round-trips to 2. `logAudit()` and the `syncOverallStatus`/`tryUnlockWebTeam` pair a few lines above were already correctly fire-and-forget — only this one notify block had the mismatch.

Not changed: `TaskBoardOverview.js`'s `handleReject()` has a similar (actually longer — 4 sequential awaited calls) chain, including a `teamUsers` lookup with the exact same "should be fire-and-forget but isn't" shape. Left as-is since it wasn't the reported symptom and `syncOverallStatus` in that path needs more careful ordering against `onRefresh()` than a drive-by fix should risk — worth revisiting if reject turns out to have the same complaint.

## Dashboard sometimes shows zero results and logs the user out (2026-08-11)

Two separate bugs, both variants of "a request was just slow, so treat it as broken." Same underlying condition (Free-tier Supabase cold-start latency) very plausibly triggers both at once, which is why they were reported together.

**1. Dashboard shows a false "No requests yet" empty state.** `Dashboard.js`'s `fetchRequests()` makes up to ~5 sequential Supabase round-trips (main `requests` query → tasks → task-progress → pending-changes → comments/team-members), each individually subject to `supabase.js`'s 10s fetch-abort. A separate `hardTimeout` in `fetchRequests()` was *also* set to 10s and unconditionally flipped `loading` to `false` — meaning under any slowness (cold start being the obvious candidate), this timeout could fire *while the real chain was still legitimately in flight*, before `setRequests(rows)` had ever been called. The UI renders `loading ? <Spinner/> : rows.length === 0 ? <EmptyState/> : <Table/>`, so the result was a confident "No requests yet" shown for however long the real fetch took beyond 10s — which then silently corrected itself once the real data arrived, but by then the user had already seen (and reasonably distrusted) an empty dashboard.

Fix: parallelized the two independent trailing queries (return-comment counts, team members for leads) via `Promise.all` instead of sequential awaits, cutting the real critical path down by one round-trip. Bumped `hardTimeout` from 10s to 30s — it's meant purely as a runaway-spinner safety net, not a normal-case timer, so it should be well above what the real (now-shorter) chain needs even under cold-start conditions.

**2. Users get force-logged-out on a slow (not invalid) session check.** `page.js`'s `INITIAL_SESSION`/`SIGNED_IN` handlers raced `getUserProfile()` against an 8-second timeout, and on that race's timeout branch, called `clearSupabaseStorage()` + `supabase.auth.signOut()` — destroying a session that was never actually invalid, purely because one profile fetch was slow. Fixed to distinguish the two outcomes: `getUserProfile()` completing and finding no matching row (a real "no such user" case) still signs out; the race's own timeout branch now just shows the login screen without touching the stored session, so a retry/refresh can still succeed without ever having forced a real logout. Also bumped the race timeout from 8s to 15s, since 8s was already shorter than `supabase.js`'s own 10s per-request abort — the race could give up before the underlying fetch even had a chance to finish.

Known residual edge case, not fixed here: `getUserProfile()` itself can also return `null` after exhausting its own internal retries following a genuine network failure (not just "no profile found") — that path still triggers the harder sign-out cleanup. Distinguishing "no profile" from "network failure after retries" would need `getUserProfile()` to return a typed result instead of `null`/row — worth doing if force-logouts are still seen after this fix.
