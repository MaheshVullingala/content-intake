  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      const [{ data: r }, { data: u }, { data: h }] = await Promise.all([
        supabase.from("requests").select("*, users(name,role)").order("updated_at", { ascending: false }),
  ...
  return (
    <div className="fade-in" style={{ fontFamily: "'Rubik', sans-serif" }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ color: "#B5B5B5", fontSize: 14 }}>Monitor requests, manage users and assign roles</p>
      </div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ background: "#F9F9F9" }}>
                    {["Title","Type","Status","Submitted By","Created","Updated"].map(h => (
                  ...
                  {requests.map((r, i) => {
                    ...
                    return (
                      <tr key={r.id} style={{ borderBottom: i < requests.length - 1 ? "1px solid #F9F9F9" : "none" }}>
                        ...
                        <td style={{ padding: "0.75rem 1rem", fontSize: 11, color: "#B5B5B5" }}>{new Date(r.created_at).toLocaleDateString()}</td>
          ...
          {tab === "users" && (
            <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #E0E0E0", overflow: "hidden" }}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ background: "#F9F9F9" }}>
                    {["User","Email","Department","Role","Joined","Action"].map(h => (
                      <th key={h} style={{ padding: "0.65rem 1rem", textAlign: "left", fontSize: 10, color: "#B5B5B5", fontWeight: 500, letterSpacing: "0.08em", textTransform: "uppercase", borderBottom: "1px solid #F3F3F3" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                  ...
                  {users.map((u, i) => {
                    ...
                    return (
                      <tr key={u.id} style={{ borderBottom: i < users.length - 1 ? "1px solid #F9F9F9" : "none", background: isPending ? "#fffdf5" : "#fff" }}>
                        ...
                        <td style={{ padding: "0.75rem 1rem" }}>
                          {u.id !== user.id ? (
                            <select value={u.role} onChange={e => assignRole(u.id, e.target.value)} disabled={updating === u.id}
