...
export default function TaskPanel({ task, req, user, attachments = [], onClose, onRefresh, inline = false }) {
  ...
  const panelInner = (
    <div className={inline ? styles.panelInline : styles.panel}>
      ...
      <div className={styles.panelHeader}>
        ...
        {!inline && <button className={styles.closeBtn} onClick={onClose}>×</button>}
      </div>
      ...
      <div className={styles.panelBody}>
        ...
        <div className={styles.actionBar}>
          ...
          {isWebTeam && (isMyTask || isAdmin) && task.status === "pending" && (
            <button
              className={`${styles.btn} ${styles.btnPrimary}`}
              onClick={markInProgress}
              disabled={saving}
