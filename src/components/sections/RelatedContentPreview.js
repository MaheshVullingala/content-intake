          ...
          {rc_cards.map((card, i) => (
            <div key={card.id || i} style={{ border: "1px solid #E0E0E0", borderRadius: 8, overflow: "hidden", background: "#fff" }}>
              ...
              <div style={{ height: 180, background: "#F3F3F3", overflow: "hidden", position: "relative" }}>
                {card.image_url
                  ? <img src={card.image_url} alt={card.title} style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={e => { e.target.style.display = "none"; }} />
              ...
              <div style={{ padding: "1rem" }}>
                ...
                {card.description && (
                  <p style={{ fontSize: 13, color: "#646464", lineHeight: 1.6, marginBottom: 12, display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                    {card.description}
                  </p>
                )}
                {card.link && (
