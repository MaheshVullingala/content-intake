...
function CarouselPreview({ data = {} }) {
  ...
  return (
    <div style={{ marginBottom: 32 }}>
      {data.title && (
        <h3 style={{ fontSize: 18, fontWeight: 400, color: "#181313", marginBottom: 24, textAlign: "center" }}>
          {data.title}
        </h3>
      )}
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 32 }}>
      {data.r1_title && (
        <div style={{ background: "#F3F3F3", borderRadius: 8, overflow: "hidden", position: "relative", minHeight: 220 }}>
          {data.r1_image && (
            <img src={data.r1_image} alt={data.r1_title}
          ...
          <div style={{ position: "absolute", top: 12, right: 12 }}>
            {data.r1_tag && (
              <span style={{ background: "rgba(255,255,255,0.9)", color: "#181313", fontSize: 10, fontWeight: 600, padding: "3px 8px", borderRadius: 4, textTransform: "uppercase", letterSpacing: "0.06em" }}>
                {data.r1_tag}
              </span>
            )}
          </div>
        ...
        {[2, 3, 4].map(n => data[`r${n}_title`] && (
          <div key={n} style={{ border: "1px solid #E0E0E0", borderRadius: 8, padding: "0.9rem 1rem", display: "flex", flexDirection: "column", gap: 6 }}>
            <div style={{ display: "flex", justifyContent: "flex-end" }}>
              {data[`r${n}_tag`] && (
                <span style={{ fontSize: 10, fontWeight: 600, color: "#646464", textTransform: "uppercase", letterSpacing: "0.06em", background: "#F3F3F3", padding: "2px 7px", borderRadius: 4 }}>
                  {data[`r${n}_tag`]}
                </span>
              )}
            </div>
...
export default function ResourcesPreview({ data = {} }) {
  ...
  return (
    <div style={{ background: "#F9F9F9", width: "100%", boxSizing: "border-box", fontFamily: "'Rubik', sans-serif" }}>
      <div className="section-container">
        ...
        {res_impact && (
          <h2 style={{ fontSize: 28, fontWeight: 400, color: "#181313", lineHeight: 1.3, marginBottom: 40, textAlign: "center", wordBreak: "break-word" }}>
            {res_impact}
          </h2>
        )}

        {res_selected.includes("video_carousel") && <CarouselPreview data={res_video_carousel} />}
        {res_selected.includes("mixed_carousel") && <CarouselPreview data={res_mixed_carousel} />}
